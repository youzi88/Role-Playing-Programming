	...
	if (turn == PLAYER_TURN) {
		...
		switch (status) {
		  ...
		  case STATUS_MAGIC_ENEMY:
			// 点选时的魔法范围
			if (MapRect.PtInRect(pos) && (map_data[pos.y][pos.x].type
                            & (ATTACKDIST | ENEMY)) == (ATTACKDIST | ENEMY)) {
		  ...
		  case STATUS_MAGIC_PLAYER:
			// 点选时的魔法范围
			if (MapRect.PtInRect(pos) && (map_data[pos.y][pos.x].type
                            & (ATTACKDIST | PLAYER)) == (ATTACKDIST | PLAYER)) {
				Magic(SelChar, (CCharacter *)map_data[pos.y][pos.x].sprite, true);
				SelChar->attack_done = true;
			}
			break;
...
void CBattleAction::ChangeStatus(CPoint point)
{
	if (MapRect.PtInRect(point) && map_data[point.y][point.x].type & (PLAYER | ENEMY)) {
		if (StatusChar != (CCharacter *)map_data[point.y][point.x].sprite) {
			...
			PopupImage.Copy(&PopupParts, CRect(21, 17, 21 + hp_len, 20),
                            CPoint(0, 20));
			PopupImage.Copy(&PopupParts, CRect(21, 32, 21 + mp_len, 35),
                            CPoint(0, 23));
			...
			for (int i=0; i<3; i++) {
				PopupImage.Copy(&PopupParts,
                                    CRect(42 + i * 8, 21, 50 + i * 8, 31),                                                              CPoint((hp_str[i] - '0') * 8, 0));
				PopupImage.Copy(&PopupParts,
                                    CRect(74 + i * 8, 21, 82 + i * 8, 31),                                                              CPoint((mhp_str[i] - '0') * 8, 0));
				PopupImage.Copy(&PopupParts,
                                    CRect(42 + i * 8, 36, 50 + i * 8, 46),                                                              CPoint((mp_str[i] - '0') * 8, 10));
				PopupImage.Copy(&PopupParts,
                                    CRect(74 + i * 8, 36, 82 + i * 8, 46),                                                              CPoint((mmp_str[i] - '0') * 8, 10));
			}
